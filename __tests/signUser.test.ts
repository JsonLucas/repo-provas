...
import supertest from "supertest";
    ...
    describe('POST /sign-in', () => {
        it('sign in request should return 200', async () => {
            const login = { email: 'teste_2@email.com', password: '123' }; //success
            const { status, body } = await supertest(app).post('/sign-in').send(login);
            expect(status).toEqual(200);
            expect(body).toMatchObject;
        });
        it('sign in request should return 400', async () => {
            const body = { email: 'teste_2@email.com', password: '1234' }; //wrong credentials
